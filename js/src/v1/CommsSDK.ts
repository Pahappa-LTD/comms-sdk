...
export class CommsSDK {
    ...
    public async sendSMS(
        ...
        const apiResponse = await this.querySendSMS(numbersArray, message, senderId, priority);
    ...
    private async sdkNotAuthenticated(): Promise<boolean> {
        if (!this._isAuthenticated) {
            console.error('SDK is not authenticated. Please authenticate before performing actions.');
            console.error('Attempting to re-authenticate with provided credentials...');
